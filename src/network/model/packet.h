...
public:
     ...
     *
     * \brief The index is an offset from the start of the packet.Êý¾Ý°ü¿ªÊ¼µÄÆ«ÒÆÁ¿
     */
    uint32_t GetStart (void) const;//Êý¾Ý°ü¿ªÊ¼µÄÎ»ÖÃ
    /**
     * \returns the index of the last byte tagged by this tag.
     *
     * \brief The index is an offset from the start of the packet.
     */
    uint32_t GetEnd (void) const; //Êý¾Ý°ü½áÊøµÄÎ»ÖÃ
    /**
     * \brief Read the requested tag and store it in the user-provided tag instance.
     *   ÇëÇó±êÇ©²¢½«Æä´æ´¢ÔÚÓÃ»§Ìá¹©µÄ±êÇ©ÊµÀýÖÐ¡£
     * \param tag the user tag to which the data should be copied.
     *
     * This method will crash if the type of the tag provided
     * by the user does not match the type of the underlying tag. Èç¹ûÓÃ»§Ìá¹©µÄ±êÇ©ÀàÐÍÓëµ×²ã±êÇ©µÄÀàÐÍ²»Æ¥Åä£¬´Ë·½·¨½«»á±ÀÀ£¡£
     */
...
/**
 * \ingroup packet
 * \brief Iterator over the set of packet tags in a packet µü´úÆ÷±éÀúÊý¾Ý°üÖÐµÄÒ»×éÊý¾Ý°ü±ê¼Ç
 *
 * This is a java-style iterator.
 */
 ...
 * were serialized in the byte buffer. The maintenance of metadata£¨ÔªÊý¾Ý£© is
 ...
 * pretty easy and is a matter of creating a ssubclas of the ns3::Header 
...
public:
   ...
   * packet. ÔÚµ±Ç°Êý¾Ý°üµÄÄ©Î²Á¬½ÓÊäÈëÊý¾Ý°ü¡£
   *
